...
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; // инициализация статического мьютекса
...
void print_shared_memory(struct shared_data_original *data,   // указатель на структуру shared_data_original, содержащую данные, записываемые в разделяемую память
                         int segment_half)                    // номер сегментной половинки разделяемой памяти (1 или 2) 
{
    // захватываем мьютекс для безопасного доступа к потоку вывода - защита от вероятности
    // "раздробленного" вывода на экран
    ...
    // освобождаем мьютекс после завершения работы с потоками вывода - данные вывелись
